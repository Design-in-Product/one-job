...
describe('v1 → v3 migration over the fixture corpus', () => {
  for (const [name, fixture] of Object.entries(allFixtures)) {
    it(`${name}: preserves every card and every nested task`, () => {
      ...
      expect(doc.decks[0].cards).toHaveLength((fixture as unknown[]).length);
      ...
      expect(countV2(doc.decks[0].cards)).toBe(countV1(fixture as any[]));

      // no card carries the old shape forward
      for (const card of doc.decks[0].cards) {
  ...
  it('substack names become interior deck names; empty substacks become empty decks', () => {
    ...
    const [parent] = doc.decks[0].cards;
  ...
  it('treats garbage shapes as empty (never throws, never invents data)', () => {
    for (const junk of [null, undefined, { not: 'a deck' }, 42]) {
      expect(migrateDocument(junk).decks.flatMap(d => d.cards)).toEqual([]);
    }
  });
});
