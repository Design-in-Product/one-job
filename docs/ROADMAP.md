...
| P | What | State |
|---|---|---|
| P0 | **Submit 1.0 to the App Store from the native build** | ✅ **APPROVED 2026-09-08** (submitted 09-05, three-day review). Manual release — live when Xian presses the button. External TestFlight now available: un-gates the 50-user cohort on native builds, no PWA fix required. |
...
| R2.2b | **Wide-viewport peeks — the hint becomes a card outline** (Xian, 2026-09-05, from reviewing the iPad store shots). Today `CanvasPeek` is a 14px sliver pinned to the CONTAINER edge (`max-w-md mx-auto`). On a phone that edge IS the screen edge, so the sliver works by implication: it is clipped, therefore something continues. On iPad the container floats in a wide field, nothing clips it, and the implication collapses — it reads as a stray grey bar. His proposal: on viewports that float in horizontal space, continue the hint out to the **full card outline**, with a **gradient fade so nothing further is expected past it**. Constraints agreed in discussion: (a) outline + identity hue only, never card content — the moment it previews content it becomes the postponed R2 canvas view arriving disguised as polish; (b) the fade must TERMINATE rather than soften, since a hard edge implies another card past it, and another. Open question: does a full outline still read as *the afterlife* on the grey variant without a label, or does the shape make the rooms look like just another deck? The fade profile may have to do that work. | iPad review |
