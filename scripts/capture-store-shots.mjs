...
for (const name of targets) {
  ...
  // Clear metrics first (2026-09-04, caught mid-upload): R1.5's "Your
  // usage" panel only renders once something has been recorded, and the
  // capture's own interactions record. The iPad viewport is tall enough
  // to reach the panel, so the shot was showing counts generated by this
  // automation — non-deterministic run to run, nonsensical to a stranger
  // ("Cards created: 0, Cards deferred: 1"), and sitting directly under a
  // caption that reads "No account, no cloud, no tracking."
  // A new user's Settings has no usage panel, so that is what we show.
  await page.evaluate(() => localStorage.removeItem('oneJobMetrics'));
  await holdMenu();
